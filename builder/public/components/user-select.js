class UserSelectComponent extends Formio.Components.components.select {
    ...
    async loadItems(url, input, callback) {
        ...
        const users = this.component.includeUsers ? await fetch(`/identityprovider/scim/Users?count=${count}&startIndex=${startIndex}`).then(async response => await response.json()) : { resources: [] };
        const groups = this.component.includeGroups ? await fetch(`/identityprovider/scim/Groups?count=${count}&startIndex=${startIndex}`).then(async response => await response.json()) : { resources: [] };
        ...
        switch (format) {
            case 'identity':
                ...
                const u1 = users.resources.map(user => ({ label: user.displayName, value: user.id }));
                const g1 = groups.resources.map(group => ({ label: group.displayName, value: group.id }));
            ...
            case 'id':
                //.resources[0].id
                const u2 = users.resources.map(user => ({ label: user.id, value: user.id }));
                const g2 = groups.resources.map(group => ({ label: group.id, value: group.id }));
            ...
            case 'object':
                //.resources[0]
                const u3 = users.resources.map(user => ({ label: user, value: user.id }));
                const g3 = groups.resources.map(group => ({ label: group, value: group.id }));
